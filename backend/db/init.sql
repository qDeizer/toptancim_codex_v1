-- Bu betik, veritabanını ve gerekli tabloları oluşturur.
-- psql -U postgres -d toptancimdb_codex -f backend/db/init.sql

-- 'toptancimdb_codex' adında bir veritabanı yoksa oluşturur.
SELECT 'CREATE DATABASE toptancimdb_codex'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'toptancimdb_codex')\gexec

-- Oluşturulan veritabanına bağlanır.
\c toptancimdb_codex;
